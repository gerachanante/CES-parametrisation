...
STAGE_TAG <- "stage1"  # or "stage2" when you analyse stage 2
ces_file  <- paste0("ces_", STAGE_TAG, "_objects.rds")
...
# For failure reason classification
...
econ_fail_reason <- function(delta_KVA, delta_VAY, gamma, nu, lambda){
  delta_KVA <- suppressWarnings(as.numeric(delta_KVA))
  delta_VAY <- suppressWarnings(as.numeric(delta_VAY))
  gamma     <- suppressWarnings(as.numeric(gamma))
  nu        <- suppressWarnings(as.numeric(nu))
  lambda    <- suppressWarnings(as.numeric(lambda))
  
  dplyr::case_when(
    !is.finite(delta_KVA) | delta_KVA < 0 | delta_KVA > 1 ~ "δK–VA out of [0,1]",
    !is.finite(delta_VAY) | delta_VAY < 0 | delta_VAY > 1 ~ "δVA–Y out of [0,1]",
...
# 1.1 Coverage per (region, method)
cov_region <- rg %>%
  group_by(r, method) %>%
  summarise(
    share_converged = mean(conv,  na.rm = TRUE),
...
# Order regions by average coverage (for tidy display)
ord_r <- rm_share %>%
...
fig1_4 <- results_grid %>%
  ...
  stat_summary(
    ...
    shape = 21, fill = NAVY, colour = NAVY
  ) +
  coord_flip() +
  labs(
...
# 1.5 Runtime vs fit quality (only converged runs)
...
# 1.6 RSS vs iterations (global quadratic)
...
method_levels <- unique(c(
  if (exists("method_order")) method_order else character(0),
...
fig1_6 <- ggplot(fit_df, aes(x = iter, y = rss, colour = method_f)) +
...
# 1.7 ΔAICc surfaces: edge minima share by method
...
fig1_7 <- ggplot(edge_counts, aes(x = reorder(method, share_edge), y = share_edge)) +
...
# 1.9 Failure reasons by method (invalid runs only + non-conv)
...
# 2) Diagnose which econ rule failed (ONLY delta_KVA / delta_VAY)
econ_fail_reason <- function(delta_KVA, delta_VAY, gamma, nu, lambda){
  delta_KVA <- suppressWarnings(as.numeric(delta_KVA))
  delta_VAY <- suppressWarnings(as.numeric(delta_VAY))
  gamma     <- suppressWarnings(as.numeric(gamma))
...
runs_status <- results_grid %>%
  mutate(
    status = dplyr::case_when(
      ...
      conv & !valid ~ econ_fail_reason(delta_KVA, delta_VAY, gamma, nu, lambda),
      conv &  valid ~ "Valid",
      TRUE          ~ "Unspecified"
    ),
    status = factor(
      ...
      levels = c(
        ...
        # econ-invalid reasons
        "δK–VA out of [0,1]","δVA–Y out of [0,1]","γ out of (0.2,5)",
...
# 4) Summarise to counts per method × status
...
# 5) Order methods by number of non-valid runs (most problematic first)
method_order_err <- error_summary2 %>%
  group_by(method) %>%
...
# 6) Plot: stacked bars (blue = valid, other colours = failure modes)
fig1_9 <- ggplot(error_summary2, aes(x = method, y = n, fill = status)) +
  ...
  labs(
    ...
    subtitle = "Stacked bars: valid runs vs solver non-convergence and economic bounds failures",
...
# ===================== 4. STAGE-2: VALID RUNS DIAGNOSTICS ===================== #
# Here we look only at runs that are converged + econ-valid.
...
# 2.2 distributions overlaid (same x-axis), by method and overall
rho_medians <- results_grid_valid %>%
  select(method, rho_KL, rho_VAE) %>%
  pivot_longer(c(rho_KL, rho_VAE),
               names_to  = "rho_type",
               values_to = "rho") %>%
  filter(is.finite(rho)) %>%
  mutate(
    rho_type = recode(rho_type,
                      rho_KL  = "ρK–L",
                      rho_VAE = "ρVA–E")
  ) %>%
  group_by(method, rho_type) %>%
...
rho_summary <- results_grid_valid %>%
  select(method, rho_KL, rho_VAE) %>%
  pivot_longer(c(rho_KL, rho_VAE),
               names_to  = "rho_type",
               values_to = "rho") %>%
  ...
  group_by(method, rho_type) %>%
  summarise(
    q10 = quantile(rho, 0.10, na.rm = TRUE),
    q25 = quantile(rho, 0.25, na.rm = TRUE),
    med = quantile(rho, 0.50, na.rm = TRUE),
    q75 = quantile(rho, 0.75, na.rm = TRUE),
    q90 = quantile(rho, 0.90, na.rm = TRUE),
    .groups = "drop"
  )

fig_rho_interval <- rho_summary %>%
  ggplot(aes(x = med, y = fct_rev(method))) +
  geom_linerange(aes(xmin = q10, xmax = q90),
                 linewidth = 0.4, colour = "grey75") +
  geom_linerange(aes(xmin = q25, xmax = q75),
                 linewidth = 0.8, colour = "grey40") +
  geom_point(size = 1.9, colour = "black") +
  facet_grid(rho_type ~ ., scales = "free_x") +
  labs(
    title   = expression("ρ medians and dispersion by method (valid runs)"),
    x       = expression(rho),
    y       = NULL,
    caption = "Dots: median ρ. Thick lines: 25–75% range. Thin lines: 10–90% range."
  ) +
  theme_nat() +
  theme(
...
print(fig_rho_interval)



fig2_2b <- results_grid_valid %>%
...
elas_long <- results_grid_valid %>%
  ...
  mutate(
    which  = recode(which, "sigma_KL"="σK-L","sigma_VAE"="σVA–E"),
    method = factor(method, levels = method_order)
  ) %>%
  filter(is.finite(sigma))
...
elas_stats <- elas_long %>%
  ...
  summarise(
    ...
    q3  = quantile(sigma, .75, na.rm = TRUE),
    .groups = "drop"
  )

order_by_vae <- elas_stats %>%
  filter(which == "σVA–E") %>%
...
fig2_3 <- ggplot(elas_stats, aes(x = factor(method, levels = order_by_vae), y = med, fill = which)) +
  ...
  geom_text(
    aes(label = paste0("median ", round(med,3), "\n[", round(q1,2), ", ", round(q3,2), "]")),
    position = position_dodge(width = .7),
    hjust    = -0.1,
    vjust    = 0.5,
    size     = 3.5,
  ...
  labs(
    ...
    x = NULL, y = expression(sigma), fill = NULL, caption = cap_elast
  ) +
  theme_nat()
print(fig2_3)
...
best_params_long <- best_methods %>%
  select(r, gamma, lambda, nu, delta_KVA, delta_VAY) %>%
  ...
  mutate(
    ...
    # clamp ONLY the share parameters
    val = ifelse(param %in% c("δK–VA","δVA–Y"), pmin(pmax(val, 0), 1), val)
  ) %>%
...
fig2_4 <- ggplot(best_params_long, aes(x = param, y = val)) +
  ...
  stat_summary(
    ...
    aes(label = round(..y.., 2)),
    vjust = -0.6, colour = NAVY, size = 2.8
  ) +
...
if (ncol(cor_df) >= 2) {
  ...
  new_names <- c("γ","λ","ν","δK–VA","δVA–Y","σK–L","σVA–E")
  if (length(new_names) == ncol(cor_mat)) {
...
# 2.6 Residual SD by region (if time-level file exists)
if (file.exists("CES_results_time.csv")) {
  ...
  fig2_6 <- results_table_time %>%
    ...
    geom_boxplot(fill = LGREY, colour = DGREY,
                 outlier.size = 1.2, outlier.alpha = .5) +
    geom_text(
      data = res_by_reg,
      aes(x = r, y = Inf, label = paste0("n=", n)),
...
# 2.7 Fit quality by method (valid runs, RSS)
rss_by_method <- results_grid_valid %>%
  group_by(method) %>%
  summarise(
    med_rss = median(rss, na.rm = TRUE),
    iqr_rss = IQR(rss, na.rm = TRUE),
    .groups = "drop"
  )
...
fig2_7 <- ggplot(rss_by_method, aes(x = reorder(method, med_rss), y = med_rss)) +
  geom_col(fill = LGREY, colour = "white", width = 0.65) +
  geom_errorbar(
    aes(ymin = med_rss - iqr_rss/2, ymax = med_rss + iqr_rss/2),
    width = .2, colour = DGREY
  ) +
  geom_text(
    aes(label = paste0("median ", round(med_rss,2),
                       " | IQR=", round(iqr_rss,2))),
    hjust = -0.05, vjust = -1, size = 3.5, colour = DGREY
  ) +
  coord_flip() +
  labs(
    title    = "Fit quality by method (RSS)",
    subtitle = "Median ± IQR of log(RSS) for valid runs",
    x = NULL, y = "Median log(RSS)"
  ) +
  theme_nat()
print(fig2_7)

# ===================== 5. STAGE-3: BEST METHODS & UNCERTAINTY ===================== #
...
fig3_1 <- best_methods %>%
  ...
  coord_flip(ylim = c(0, 1.05)) +
  labs(
    title = "Best method by region",
    x = NULL, y = "Share of regions"
  ) +
  theme_nat()
print(fig3_1)

# 3.2 R² by region (best method), with global median + map
...
fig3_2b <- ggplot(world_best) +
  ...
  scale_fill_gradient(
    limits = c(0, 1), low = "white", high = NAVY, na.value = "grey90"
  ) +
  labs(
    title = expression("Map of fit quality ("*R^2*") — best methods"),
    fill  = expression(R^2)
  ) +
  theme_nat()
print(fig3_2b)
...
fig3_4 <- ggplot() +
  geom_linerange(
    ...
    linewidth = 2.2, colour = LGREY, alpha = .7
  ) +
  geom_point(
    data = elas_reg,
...
if (file.exists("CES_results_time.csv")) {
  ...
  fig3_5 <- obs_fit %>%
    ...
    labs(
      ...
      x        = "Fitted (scaled)", y = "Observed (scaled)", caption = cap_fit
    ) +
    theme_nat()
  print(fig3_5)
} else {
...
fig3_6 <- ggplot(world_best) +
  ...
  labs(
    title = expression(paste(sigma[K-L], " by region")),
...
fig3_7 <- ggplot(world_best) +
  ...
  scale_fill_gradient(low="#FFC55D", high=NAVY, na.value="grey90") +
  labs(
    title   = expression(paste(sigma[VA-E], " by region")),
...
ciw <- best_methods %>%
  transmute(
    ...
    ciw_dKVA   = ci_hi_delta_KVA - ci_lo_delta_KVA,
    ciw_dVAY   = ci_hi_delta_VAY - ci_lo_delta_VAY,
    ciw_nu     = ci_hi_nu        - ci_lo_nu
  )

ciw_long <- ciw %>%
  pivot_longer(-c(r, method), names_to = "param", values_to = "ciw") %>%
  mutate(
    param = recode(
      param,
      ciw_gamma  = "γ",
...
fig3_9 <- ggplot(ciw_long, aes(x = param, y = ciw)) +
  ...
  labs(
    ...
    x     = NULL, y = "Width (95% approx = 1.96·SE*2)"
  ) +
  theme_nat()
print(fig3_9)
...
fig3_10a <- ggplot(p_long2, aes(x = p)) +
  ...
  labs(
    title = "P-value distributions (best methods)",
    x     = "p-value", y = "Count"
  ) +
  theme_nat()
print(fig3_10a)
...
fig3_11 <- ggplot(p_long, aes(x = parameter, y = r, fill = class)) +
  ...
  geom_text(
    aes(label = ifelse(is.na(p), "n/a", scales::pvalue(p, accuracy = .001))),
...
# 3.12 Stability of elasticities across grid (per region IQR)
grid_bestmethod <- results_grid %>%
...
# 3.13 Parameter estimation with uncertainty and significance (tiles + SE + p)
...
fig3_13 <- ggplot(param_summary,
  ...
  scale_fill_gradient2(
    low = DGREY, mid = "white", high = NAVY, midpoint = 0
  ) +
  labs(
...
param_vars <- c("gamma","lambda","nu","delta_KVA","delta_VAY","sigma_KL","sigma_VAE")
...
fig3_15a <- ggplot(best_rho_long, aes(x = rho, fill = rho_type)) +
  ...
  scale_fill_manual(
    values = c("ρKL" = MGREY, "ρVA–E" = NAVY),
    breaks = names(legend_labs),
    labels = unname(legend_labs),
    name   = NULL
  ) +
...
# 3.15b 2D grid heatmap: where best-method cells fall in the full first-run grid
if (all(c("grid_counts","grid_base","k_vals","v_vals") %in% ls())) {
  ...
  message("Skipping rho-grid heatmap (3.15b): grid_counts/grid_base/k_vals/v_vals not found.")
}

# ===================== 6. STAGE-4: IAM-LEVEL CONSISTENCY CHECKS ===================== #

# 4.1 TFP growth vs IAM TFP trajectories
...
fig4_1b <- iam_table %>%
  ...
  labs(
    title   = "IAM input: TFP trajectories normalised to base",
    x       = "Year", y = "TFP / TFP[base]",
    caption = cap_params
  ) +
  theme_nat()
print(fig4_1b)
